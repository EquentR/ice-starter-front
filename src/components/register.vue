<template>
  <div class="reg-box">
    <p class="title">注册&nbsp;ice-starter</p>
    <input class="user-input" type="text" placeholder="用户名" v-model="user" autocomplete="off">
    <input class="user-input" type="email" placeholder="邮箱" v-model="email" name="username" autocomplete="off">
    ...
    <button @click="login" class="submit">返回登录</button>
    <button @click="regist" class="register">注册</button><br>
  ...
  methods: {
    ...
    regist() {
      if (this.user !== '' &&
        ...
        userApi.register(user, this.code)
          .then(
            response => {
              this.$notify({
                title: '成功',
